...
public class AddContactActivity extends BaseActivity{
	...
	public void addContact(View view){
		if(InvisibleLoversApplication.getInstance().getUserName().equals(nameText.getText().toString())){
			startActivity(new Intent(this, AlertDialog.class).putExtra("msg", "不能添加自己"));
			return;
		}
		
		if(InvisibleLoversApplication.getInstance().getContactList().containsKey(nameText.getText().toString())){
				...
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
							...
							Toast.makeText(getApplicationContext(), "发送请求成功,等待对方验证", Toast.LENGTH_LONG).show();
						}
					});
					...
					runOnUiThread(new Runnable() {
						public void run() {
							progressDialog.dismiss();
							Toast.makeText(getApplicationContext(), "请求添加好友失败:" + e.getMessage(), Toast.LENGTH_LONG).show();
						}
					});
				}
			}
