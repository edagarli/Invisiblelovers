package me.edagarli.invisiblelovers.db;
...
public class DatabaseUtil {
	 private static final String TAG="DatabaseUtil";
	    ...
	    public void deleteFav(QiangYu qy){
	    	Cursor cursor=null;
	    	String where = DBHelper.FavTable.USER_ID+" = '"+ InvisibleLoversApplication.getInstance().getCurrentUser().getObjectId()
	    			+"' AND "+ DBHelper.FavTable.OBJECT_ID+" = '"+qy.getObjectId()+"'";
	    ...
	    public long insertFav(QiangYu qy){
            ...
            if(cursor != null && cursor.getCount() > 0) {
		    	...
		    	ContentValues cv = new ContentValues();
		    	cv.put(DBHelper.FavTable.USER_ID, InvisibleLoversApplication.getInstance().getCurrentUser().getObjectId());
		    	cv.put(DBHelper.FavTable.OBJECT_ID, qy.getObjectId());
		    	cv.put(DBHelper.FavTable.IS_LOVE, qy.getMyLove()==true?1:0);
		    	cv.put(DBHelper.FavTable.IS_FAV,qy.getMyFav()==true?1:0);
	    ...
	    public List<QiangYu> setFav(List<QiangYu> lists) {
	        ...
	        if(lists != null && lists.size() > 0) {
	            for(Iterator iterator=lists.iterator(); iterator.hasNext();) {
	            	...
	            	String where = DBHelper.FavTable.USER_ID+" = '"+InvisibleLoversApplication.getInstance().getCurrentUser().getObjectId()//content.getAuthor().getObjectId()
